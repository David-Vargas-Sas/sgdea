package com.sgdea.multitenancy.multitenancy.role.domain.exceptions;
...
@RestControllerAdvice(basePackages = "com.sgdea.multitenancy.multitenancy.entrypoints.role")
public class RoleExceptionHandler {
    ...
    public ResponseEntity<ApiResponseDto<Void>> handleValidation(MethodArgumentNotValidException exception) throws Exception {
        Map<String, String> errors = new LinkedHashMap<>();
        exception.getBindingResult().getFieldErrors().forEach(error -> errors.put(error.getField(), error.getDefaultMessage()));
        return ResponseEntity.badRequest().body(ApiResponseDto.error("La solicitud de rol tiene campos invalidos", errors));
    }
}
